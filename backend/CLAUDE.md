...
`internal/settings` reads `config/settings.json` (path is `../config/settings.json`, relative to the `backend/` working directory) and serves it at `GET /api/settings`. `GET /api/tasks` takes an optional `?project_id=` filter so the frontend only fetches tasks for active projects. See root CLAUDE.md for the config file format and frontend/CLAUDE.md for how the frontend uses this.
